...
| azur | 225° | `#00a8d4` | 0.117 (special) | cian eléctrico, bajo los estados |
| cielo | 250° | `#b4d8ff` | 0.125 (special) | azul polvo, sobre los estados |

Los azules existen gracias a un override por tema de las paradas del acento (`stops` en
`THEMES`): a la luminosidad estándar (L 0.72) el arco 195°–265° pertenece a `info` y `special`,
así que un acento azul solo cabe esquivándolos en luminosidad. Hay exactamente dos bolsillos y
cada uno tiene su tema: **azur** pasa por debajo (base L 0.68, lo más oscuro que permite el
gate de 4.5:1 sobre el fondo) y **cielo** flota por encima (base L 0.87, donde el gamut sRGB
solo deja croma pastel — suave por construcción, y sube consigo el techo de la rampa). Un azul
medio, saturado y profundo a la vez, es imposible sin rediseñar los colores de estado.
